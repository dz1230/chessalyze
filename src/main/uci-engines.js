...
const fetchOptions = (engine) => {
    return new Promise((resolve, reject) => {
        ...
        const p = child_process_1.spawn(engine.exePath, {
            cwd: path_1.dirname(engine.exePath),
            stdio: 'pipe',
            timeout: 600000
        });
...
uciListeners.universal.push((engineId, command, line) => {
    if ((command === 'info') || (command === 'bestmove')) {
        ...
        const window = windows_1.getWindowByID(winId);
        window.webContents.send('uci-line', engineId, line);
    }
});
sharedstate_1.customResponseFunctions.set('uci-apply-options', async (engine, options) => {
...
sharedstate_1.customResponseFunctions.set('uci-ignite', async (engine) => {
        ...
        if (engineProcesses.has(engine.id)) {
            ...
            uciListeners.singleUse.push({
                ...
                l: (line) => {
                    ...
                    let definedOptions = engine.options ? engine.options.filter(o => options.findIndex(o1 => o.name === o1.name) && o.value !== undefined) : [];
                    for (const option of definedOptions) {
                        options.find(o1 => o1.name === option.name).value = option.value;
                    }
                    if (definedOptions.findIndex(o => o.name === 'UCI_AnalyseMode') < 0)
                        definedOptions.push({ name: 'UCI_AnalyseMode', value: 'true', default: 'false', predefined: undefined, type: 'check', max: undefined, min: undefined });
...
sharedstate_1.customResponseFunctions.set('uci-stop', async (engine) => {
    return await new Promise((resolve, reject) => {
        if (!engineProcesses.has(engine.id))
            resolve({});
        let didStop = false;
...
sharedstate_1.customResponseFunctions.set('uci-quit', async (engine) => {
    return await new Promise((resolve, reject) => {
        if (!engineProcesses.has(engine.id))
            resolve({});
        let didExit = false;
...
sharedstate_1.customResponseFunctions.set('uci-button', async (engine, command) => {
    return await new Promise((resolve, reject) => {
        uciListeners.singleUse.push({
            engineId: engine.id,
            command: 'readyok',
...
sharedstate_1.customResponseFunctions.set('uci-go', async (engine, position, newGame) => {
    return await new Promise((resolve, reject) => {
        const readyListener = {
            engineId: engine.id,
            command: 'readyok',
        ...
        const positionReadyListener = {
            ...
            l: (line) => {
                ...
                let goLine = 'go';
                if (engine.searchOptions === undefined || engine.searchOptions.infinite)
                    goLine += ' infinite';
                if (engine.searchOptions?.depth)
                    goLine += ' depth ' + engine.searchOptions.depth;
                if (engine.searchOptions?.nodes)
                    goLine += ' nodes ' + engine.searchOptions.nodes;
                if (engine.searchOptions?.movetime)
                    goLine += ' movetime ' + engine.searchOptions.movetime;
...
sharedstate_1.customResponseFunctions.set('uci-fetch-options', async (engine) => {
    return await fetchOptions(engine);
});
sharedstate_1.customResponseFunctions.set('uci-kill', async (engineId) => {
    return await new Promise((resolve, reject) => {
